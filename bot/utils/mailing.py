...
class Mailing:
    ...
    async def update_status(self):
        ...
        try:
            ...
            print(f"Error while updating status message: {e}")
    ...
    async def send_to_all(self, bot: Bot, user_ids: list[int], message: types.Message):
        for user in user_ids:
            try:
                ...
                print(f"Telegram API limit exceeded: {e}")
        ...
        for i in range(0, len(user_ids), batch_size):
            ...
            for result in results:
                if isinstance(result, TelegramRetryAfter):
                    print(f"Telegram API limit exceeded: {result}")
                    next_sleep = max(next_sleep, result.retry_after)
                elif isinstance(result, Exception):
                    self.failed += 1
                    print(f"Error while sending message: {result}")
